...
return {
  ...
  handlers = {
    ...
    form_messages = function(self, messages)
      ...
      for _, m in ipairs(messages) do
        ...
        if vim.startswith(model, "o1") and m.role == "system" then m.role = self.roles.user end
    ...
    chat_output = function(self, data, tools)
        ...
        for _, choice in ipairs(json.choices) do
          ...
          if delta and delta.tool_calls and #delta.tool_calls > 0 then
            for i, tool in ipairs(delta.tool_calls) do
              ...
              local id = tool.id
              if not id or id == "" then id = string.format("call_%s_%s", json.created, i) end
      ...
      if self.temp.extended_thinking then can_reason = true end
    ...
    inline_output = function(self, data, context)
      if self.opts.stream then return log:error "Inline output is not supported for non-streaming models" end
      ...
      if data and data ~= "" then
        ...
        local choice = json.choices[1]
        if choice.message.content then return { status = "success", output = choice.message.content } end
      end
    end,
    tools = {
  ...
  schema = {
    ...
    stop = {
      ...
      validate = function(l) return #l >= 1 and #l <= 4, "Must have between 1 and 4 elements" end,
    },
    ---@type CodeCompanion.Schema
    max_tokens = {
      order = 7,
      ...
      default = function(self)
        if
          self.schema.extended_thinking
          and self.schema.extended_thinking.default
          and self.schema.reasoning_max_tokens
          and self.schema.reasoning_max_tokens.default > 0
        then
          return self.schema.reasoning_max_tokens.default + 1000
        end
        return nil
      end,
    ...
    user = {
      ...
      validate = function(u) return u:len() < 100, "Cannot be longer than 100 characters" end,
    },
  },
}
